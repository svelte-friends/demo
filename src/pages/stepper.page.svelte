<script>
  ...
  let tbody = [
    ['labelColor', 'we use to set a color for the label.', 'String', 'Yes', '#6c757d'],
    ['completedColor', 'we use to set a color completed.', 'String', 'Yes', '#2bbbad'],
    ['activeColor', 'we use to set a color active.', 'String', 'Yes', '#624695'],
    ['stepColor', 'we use to set a color for the step.', 'String', 'Yes', '#6c757d'],
    ['numberColor', 'we use to set a color for the number.', 'String', 'Yes', '#fff'],
  ...
  let codeDemoColor = `
   <Stepper labelColor="black" completedColor="#3CB371" stepColor="#f44336" numberColor="#fff" 
    steps = {[
      { label: 'Tipo de teste', completed: true},
      { label: 'Recurso da câmera', completed: true},
      { label: 'Configuração do ambiente', completed: false},
   ]}/>
  `;
  let codeDemoIcon = `
   <Stepper completedColor="transparent" stepColor="transparent" let:step steps = {[
      { label: 'Address', completed: false, icon: house },
      { label: 'Payment', completed: false, icon: money},
    ]}>
      <span class="icon" style="width: 50px" slot="icon">
        {@html step.completed ? completed : step.icon}
      </span>
   </Stepper>

   <Stepper completedColor="transparent" stepColor="transparent" let:step steps = {[
      { label: 'STEP 1', completed: true, icon: money },
      { label: 'STEP 2', completed: true, icon: money },
      { label: 'STEP 3', completed: false, icon: medical },
      { label: 'STEP 4', completed: false, icon: contact },
    ]}>
      <span class="icon" style="width: 50px" slot="icon">
        {@html step.completed ? completed : step.icon}
      </span>
  ...
  let completed  = `<svg viewBox="0 0 512 512"><path d="M170.718 216.482L141.6 245.6l93.6 93.6 208-208-29.118-29.118L235.2 279.918l-64.482-63.436zM422.4 256c0 91.518-74.883 166.4-166.4 166.4S89.6 347.518 89.6 256 164.482 89.6 256 89.6c15.6 0 31.2 2.082 45.764 6.241L334 63.6C310.082 53.2 284.082 48 256 48 141.6 48 48 141.6 48 256s93.6 208 208 208 208-93.6 208-208h-41.6z"/></svg>`
...
<BasePage>
  ...
  <Example>
    <Stepper labelColor="black" completedColor="#3CB371" stepColor="#f44336" numberColor="#fff" steps = {[
    { label: 'Tipo de teste', completed: true},
    { label: 'Recurso da câmera', completed: true},
    { label: 'Configuração do ambiente', completed: false},
    ]}/>
